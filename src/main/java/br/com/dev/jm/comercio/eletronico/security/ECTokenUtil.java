...
public class ECTokenUtil {
    ...
    public static final String EMISSOR       = "JmDEV";
    public static final String TOKEN_KEY     = "01234567890123456789012345678901";
    public static final String TOKEN_HEADER  = "Bearer ";

    public static ECToken generateToken(UsuarioModel usuario){
        Key secretKey = Keys.hmacShaKeyFor(TOKEN_KEY.getBytes());
        String jwt = builder().setSubject(usuario.getLogin())
                                   .setIssuer(EMISSOR)
    ...
    public static Authentication decodeToken(HttpServletRequest request) {
        ...
        Jws<Claims> jwsClaims = parser().setSigningKey(TOKEN_KEY.getBytes()).build().parseClaimsJws(token);
